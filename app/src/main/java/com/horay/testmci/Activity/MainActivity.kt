package com.horay.testmci.Activity
...
import com.horay.testmci.Activity.MainActivity.Companion.myPager
import com.horay.testmci.Activity.MainActivity.Companion.tabsBtn
import com.horay.testmci.Adapter.TabAdapter
import com.horay.testmci.Fragment.BrowseFragment
import com.horay.testmci.Fragment.HomeFragment
import com.horay.testmci.Model.Bookmark
import com.horay.testmci.Model.Tab
import com.horay.testmci.R
import com.horay.testmci.databinding.ActivityMainBinding
import com.horay.testmci.databinding.BookmarkDialogBinding
import com.horay.testmci.databinding.MoreFeaturesBinding
import com.horay.testmci.databinding.TabsViewBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun initializeView(){
        ...
        binding.tabsBtn.setOnClickListener {
            ...
            val dialogTabs = MaterialAlertDialogBuilder(this, R.style.roundCornerDialog).setView(viewTabs)
                .setTitle("تاریخچه زبانه ها")
                .setPositiveButton("خانه"){self, _ ->
                    changeTab("Home", HomeFragment())
                    self.dismiss()
                }
                .setNeutralButton("جستجو در گوگل"){self, _ ->
        ...
        binding.settingBtn.setOnClickListener {
            ...
            dialogBinding.saveBtn.setOnClickListener {
                ...
                else Snackbar.make(binding.root, "ابتدا یک صفحه اینترنتی باز کنید\uD83D\uDE03", 3000).show()
            }
                ...
                frag?.let{
                    if(bookmarkIndex == -1){
                        ...
                        val dialogB = MaterialAlertDialogBuilder(this)
                            .setTitle("ایجاد نشانک جدید")
                            .setMessage(" : لینک نشانک${it.binding.webView.url}")
                            .setPositiveButton("افزودن"){self, _ ->
                                try{
                                ...
                                self.dismiss()}
                            .setNegativeButton("لغو"){self, _ -> self.dismiss()}
                        ...
                        val dialogB = MaterialAlertDialogBuilder(this)
                            .setTitle("حذف نشانک")
                            .setMessage(" : لینک${it.binding.webView.url}")
                            .setPositiveButton("حذف"){self, _ ->
    ...
    override fun onResume() {
        ...
        printJob?.let {
            when{
                it.isCompleted -> Snackbar.make(binding.root, " -> ذخیره شد${it.info.label}", 4000).show()
                it.isFailed -> Snackbar.make(binding.root, " -> خطا در ذخیره سازی ${it.info.label}", 4000).show()
            }
        }
    }
